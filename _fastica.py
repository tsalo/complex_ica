...
__all__ = ["complex_fastica", "ComplexFastICA"]
...
def _ica_def(X, tol, g, fun_args, max_iter, w_init):
    """Deflationary ComplexFastICA using fun approx to neg-entropy function

    Used internally by ComplexFastICA.
    """
...
def _ica_par(X, tol, g, fun_args, max_iter, w_init):
    """Parallel ComplexFastICA.

    Used internally by ComplexFastICA --main loop
    ...
    for ii in range(max_iter):
        ...
        warnings.warn(
            (
                "ComplexFastICA did not converge. Consider increasing "
...
def complex_fastica(
    ...
    *,
    ...
    algorithm : {'parallel', 'deflation'}, default='parallel'
        Specify which algorithm to use for ComplexFastICA.
    ...
    While ComplexFastICA was proposed to estimate as many sources
    ...
    >>> from sklearn.decomposition import complex_fastica
    >>> X, _ = load_digits(return_X_y=True)
    >>> K, W, S = complex_fastica(X, n_components=7, random_state=0, whiten='unit-variance')
    >>> K.shape
    (7, 64)
    >>> W.shape
    (7, 7)
...
class ComplexFastICA(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    """ComplexFastICA: a fast algorithm for Independent Component Analysis.
    ...
    --------
    >>> from sklearn.datasets import load_digits
    >>> from sklearn.decomposition import ComplexFastICA
    >>> X, _ = load_digits(return_X_y=True)
    >>> transformer = ComplexFastICA(n_components=7,
    ...         random_state=0,
    ...         whiten='unit-variance')
